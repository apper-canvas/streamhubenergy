...
const VideoCard = ({ content, onPlay, onAddToList, inMyList = false, progress = 0, className = "" }) => {
  ...
  return (
    ...
    {/* Main Image */}
    <div
        className="relative w-full aspect-video overflow-hidden rounded-lg bg-netflix-surface">
        <img
            src={content.thumbnail_c?.url || content.thumbnail_c}
            alt={content.title_c}
            className={`w-full h-full object-cover transition-all duration-500 ${imageLoaded ? "opacity-100 scale-100" : "opacity-0 scale-110"} ${isHovered ? "brightness-110" : "brightness-100"}`}
        ...
        {/* Progress Bar */}
        {progress > 0 && <div className="absolute bottom-0 left-0 right-0 h-1 bg-netflix-white/30">
            <div
        ...
        <div className="absolute top-3 right-3">
            <Badge variant="rating" className="text-xs">★ {content.rating_c}
            </Badge>
        </div>
        {/* Duration Badge */}
        ...
        <motion.div
            ...
            <div className="space-y-3">
                {/* Title */}
                <h3
                    className="text-netflix-white font-inter font-bold text-lg leading-tight line-clamp-2">
                    {content.title_c}
                </h3>
                {/* Metadata */}
                <div className="flex items-center space-x-3 text-sm text-gray-300">
                    ...
                    <span>•</span>
                    <span className="capitalize">{content.type_c}</span>
                    <span>•</span>
                    <span>{Math.floor(content.duration_c / 60)}h {content.duration_c % 60}m</span>
                </div>
                <div className="flex flex-wrap gap-1 mt-2">
                    {(content.genre_c?.split(",") || []).slice(0, 3).map(
                        genre => <Badge key={genre} variant="outline" className="text-xs py-0.5 px-2">
                            {genre}
                        </Badge>
                    )}
                </div>
                ...
                <div className="flex items-center space-x-2 pt-2">
                    <Button
                        ...
                        <ApperIcon name="Play" className="h-4 w-4 mr-1" />Play
                                      </Button>
                    <Button
                        size="icon"
                        variant="secondary"
        ...
        {!isHovered && <div className="absolute inset-0 flex items-center justify-center">
            <motion.div
